...
\usepackage{pifont}        % 带圈数字符号
\usepackage{setspace}      % 行距
\usepackage{indentfirst}   % 首行缩进
\usepackage{fvextra}       % 增强代码环境
\usepackage{etoolbox}      % 宏工具

\let\cleardoublepage\clearpage
\raggedbottom
\graphicspath{{figures/}{images/}{generated/figs/}}
...
\ctexset{
    chapter={
        format={\centering\heiti\mdseries\zihao{-3}},
...
\DeclareCaptionLabelFormat{thesisfigure}{图#2}
\DeclareCaptionLabelFormat{thesistable}{表#2}
\captionsetup{
    font={heiti,wuhao},
    labelsep=space,
    skip=6pt
}
\captionsetup[figure]{position=bottom,labelformat=thesisfigure}
\captionsetup[table]{position=top,labelformat=thesistable,font={heiti,wuhao}}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[CO]{\songti\zihao{5}赣南科技学院2026届毕业论文}  % 奇数页页眉
\fancyhead[CE]{\songti\zihao{5}\thesisauthor：\thesistitle}  % 偶数页页眉
...
\fancypagestyle{frontmatterstyle}{
    \fancyhf{}
    \fancyhead[CO]{\songti\zihao{5}赣南科技学院2026届毕业论文}
...
\begin{titlepage}
    ...
    \begin{center}
        \vspace*{2cm}
        {\heiti\zihao{1}赣南科技学院\par}
        \vspace{1.5cm}
        {\heiti\zihao{2}本科毕业设计（论文）\par}
        \vspace{2.5cm}
        {\heiti\zihao{-1}\thesistitle\par}
        \vspace{2.5cm}
        \renewcommand{\arraystretch}{1.8}
        \begin{tabular}{p{3.5cm}p{8.5cm}}
            学\hspace{2em}院： & \college \\
            专\hspace{2em}业： & \major \\
            班\hspace{2em}级： & \classno \\
            学\hspace{2em}号： & \studentid \\
            姓\hspace{2em}名： & \thesisauthor \\
            指导教师： & \supervisor \\
            完成时间： & \thesisdate \\
        \end{tabular}
        \vfill
...
{\centering\heiti\zihao{-1}\thesistitle\par}
{\centering\heiti\zihao{3}摘\hspace{1em}要\par}
...
\thispagestyle{frontmatterstyle}
{\centering\bfseries\zihao{-1}\thesistitleEN\par}
...
\thispagestyle{frontmatterstyle}
{\centering\heiti\zihao{3}目\hspace{1em}录\par}
...
第五章对全文进行总结，并对未来的工作进行展望。第一章为绪论，介绍了课题的研究背景、意义以及国内外研究现状。
...
\begin{enumerate}[label=\protect\ding{\numexpr171+\value*},leftmargin=2em,itemsep=0pt]
    \item 四级编号，加一个空格
    \item 支持串口虚拟功能
\end{enumerate}
...
\begin{figure}[htbp]
    \centering
    % 使用 figures/pictest.jpg，确保该文件存在
    \includegraphics[width=0.6\textwidth]{figures/pictest.jpg}
...
表格示例2说明：上表展示了另一个示例。表头采用黑体五号，英文为Times New Roman五号，居中显示。表与下方正文空一行。
...
效率计算公式：
\begin{equation}
    \eta = \frac{t_{a1}-t_{a2}}{t_{a1}-t_{w1}} \times 100\%
    \label{eq:efficiency}
\end{equation}

纳维-斯托克斯方程（Navier-Stokes Equation）分量形式示例：
\begin{equation}
    w\frac{\partial u}{\partial z} + u\frac{\partial u}{\partial r} - \frac{v^2}{r} = -\frac{1}{\rho}\frac{\partial p}{\partial z} + \gamma\left(\frac{\partial^2 u}{\partial z^2} + \frac{\partial^2 u}{\partial r^2} + \frac{1}{r}\frac{\partial u}{\partial r} - \frac{u}{r^2}\right)
    \label{eq:ns-equation}
\end{equation}
